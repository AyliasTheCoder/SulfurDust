...
// Function to get a random word from the JSON object
...
function generateRandomMessage() {
  let message = "";
  let currentWord = getRandomWord();
  const iterations = Math.floor(Math.random() * 25) + 1; // Random number between 1 and 10
...
async function fetchMessages() {
  try {
    ...
    for (let i = 0; i < 50; i++) {
      // Fetch 20 batches of 50 messages
    ...
    fs.writeFile("output.txt", allMessages.join("\n"), (err) => {
      if (err) throw err;
      console.log("Messages saved to output.txt");
    });
  } catch (error) {
